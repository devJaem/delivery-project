...
class ReviewService {
  ...
  getAllReview = async (restaurantId, sort) => {
    ...
    const existedrestaurant =
      await this.reviewRepository.existedRestaurant(restaurantId);
    if (!existedrestaurant)
      throw new NotFoundError(MESSAGES.RESTAURANT.GET_MORE.NOT_FOUND); //에러 고치기
  ...
  getReviewById = async (restaurantId, reviewId) => {
    ...
    const existedrestaurant =
      await this.reviewRepository.existedRestaurant(restaurantId);
    if (!existedrestaurant)
      throw new NotFoundError(MESSAGES.RESTAURANT.GET_MORE.NOT_FOUND);
    //해당 리뷰가 없을 시에
  ...
  putReview = async (restaurantId, reviewId, user, changereview) => {
    ...
    if (!existedrestaurant)
      throw new NotFoundError(MESSAGES.RESTAURANT.GET_MORE.NOT_FOUND);
    //해당 리뷰가 없을 시에
    const reviewExisted = await this.reviewRepository.getReviewById(
      restaurantId,
      reviewId,
    );
    if (!reviewExisted)
      throw new NotFoundError(MESSAGES.REVIEW.GET_MORE.NOT_FOUND);
    const review = await this.reviewRepository.putReview(
      restaurantId,
      reviewId,
      user,
      changereview,
