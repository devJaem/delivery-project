...
class RestaurantService {
  ...
  putRestaurant = async (restaurantId, user, changeRestaurant) => {
    ...
    if (!existedRestaurant)
      throw new NotFoundError(MESSAGES.RESTAURANT.GET_MORE.NOT_FOUND);
    const restaurant = await this.restaurantRepository.putRestaurant(
      restaurantId,
      user,
      changeRestaurant,
    );
  ...
  deleteRestaurant = async (restaurantId, user) => {
    const existedRestaurant =
      await this.restaurantRepository.existedRestaurant(user);
    if (!existedRestaurant)
      throw new NotFoundError(MESSAGES.RESTAURANT.GET_MORE.NOT_FOUND);
    const restaurant = await this.restaurantRepository.deleteRestaurant(
      restaurantId,
      user,
    );
    return {
