...
class ReviewController {
    ...
    getReviewById = async (req, res, next) => {
        try {
            ...
            const review = await this.reviewService.getReviewById(restaurantId, reviewId);
            return res.status(HTTP_STATUS.OK).json({
                status: HTTP_STATUS.OK,
                message: MESSAGES.REVIEW.GET_ALL.SUCCEED,
                data: review,
    ...
    createReview = async (req, res, next) => {
        try {
            ...
            const review = await this.reviewService.createReview(restaurantId, user, createreview);
            return res.status(HTTP_STATUS.OK).json({
                status: HTTP_STATUS.OK,
    ...
    putReview = async (req, res, next) => {
        try {
            ...
            const review = await this.reviewService.putReview(restaurantId, reviewId, user, changereview);
            return res.status(HTTP_STATUS.OK).json({
                status: HTTP_STATUS.OK,
    ...
    deleteReview = async (req, res, next) => {
        try {
            const { reviewId, restaurantId } = req.params;
            const user = req.user;
            const review = await this.reviewService.deleteReview(restaurantId, reviewId, user);
            return res.status(HTTP_STATUS.OK).json({
                status: HTTP_STATUS.OK,
